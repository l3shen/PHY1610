...
int main(int argc, char* argv[])
{
	...
	#pragma omp parallel for default(none) shared(tau,ngrid,c,dx,rho,rho_prev,rho_next,dt)
		...
		// Parallelizing output to single file.
		// Beging parallel computing.
		#pragma omp parallel default(none) shared(dataFile,npnts,x,rho)
		{
		
		// Only using one core for writing the trivial non-looped parts.	
		#pragma omp single
                dataFile << time << "\n"; // This is not really necessary, as was pointed out by Peteer on the forums.

		// Using multiple cores to parallelize the for loop.
		#pragma omp for
                for (int i = 0; i < npnts; i++ ) {
			#pragma omp critical 				// Does not work with atomic update - the type of operator (binary) is not compatible.
                 	dataFile << x[i] << " " << rho[i] << "\n";
		}

		// Again, use single core to parallelize the for loop.
		#pragma omp single 
                dataFile << "\n";
		}
           } 
